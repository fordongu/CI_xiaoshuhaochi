...
//保存订单
    function save_order(){
    	$uid = $this->_get_uid();
    	$event = $this->common->get_valid_event($uid);
    	$this->common->save_order($uid,WEBSITE_ORDER,'main',$event);   	
    }	
...
//用户登录
    function user_login(){
    	...
    	$remember_flag = isset($_POST['remember_flag'])?trim($_POST['remember_flag']):'';
    	...
    	if((!$mobile_check)&&(!$email_check)){
			if($_POST['check']==0){
				echo json_encode(array('success'=>'no','msg'=>'手机号不存在'));exit;
			}else{
				echo json_encode(array('success'=>'no','msg'=>'邮件或手机号不存在'));exit;
			}
    	...
    	if($md5_password != $user->tu_password){
    		echo json_encode(array('success'=>'no','msg'=>'密码错误'));exit;
    	}
    	setcookie('user_cookie',serialize($user),time()+3600*24*365,'/');
    	setcookie('remember_password',$remember_flag,time()+3600*24*365,'/');
		if($_POST['mobile_email']==0){
   		
 			
		}
    	echo json_encode(array('success'=>'yes','msg'=>'登陆成功'));exit;

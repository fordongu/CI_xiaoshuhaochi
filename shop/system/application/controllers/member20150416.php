...
//修改用户资料	
   function ajax_update_user(){
   	...
   	if($mobile_code&&($field=='tu_mobile')){
		...
		if($res["success"]=="yes"){
				$result = $this->tickets->update('users',array($field=>$val),array('tu_id'=>$uid));
   	if($result){
   		...
   		echo json_encode(array('success'=>'no1','msg'=>'修改失败'));exit;
   	}
		}else{
			echo json_encode(array('success'=>'no2','msg'=>'验证码认证失败'));exit;
	...
	$md5_password = md5(md5('xiaoshu'.$password));
   	...
   	if($result){
   		$temp_user = $this->tickets->select('users',array('tu_id'=>$uid));
   		setcookie('user_cookie',serialize($temp_user[0]),time()+3600*24*365,'/');
...
AND t_shipping_address.tsa_district = t_area.area_id and t_shipping_address.tsa_uid=".$uid." order by t_shipping_address.tsa_id desc";
    ...
    function get_default_address(){
		...
		left join t_companys on t_service_buildings.id=t_companys.service_building_id where t_shipping_address.tsa_id='$address_id' and t_shipping_address.tsa_building_id='$tsa_building_id';";

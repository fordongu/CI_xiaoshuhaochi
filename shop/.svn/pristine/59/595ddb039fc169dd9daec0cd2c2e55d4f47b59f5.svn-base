function InputHandle(elem) {
	elem = elem.nodeType === 1 ? elem : document.getElementById(elem);
	try {
		if(elem.nodeName && (elem.nodeName === 'INPUT' || elem.nodeName === 'TEXTAREA') && elem.getAttribute('data-value')) {
			this.input = elem;
			this.reset();
			var self = this;
			self.bind(elem,'focus');
			self.bind(elem,'blur');
		}
	} catch(e) {
		throw '必须传入input或textarea对象、input或textarea必须有data-value属性';
	}
}
InputHandle.prototype = {
	handleEvent : function(e) {
		e = e || window.event;
		switch(e.type) {
			case 'focus' : this.focusHandle();break;
			case 'blur' : this.blurHandle();break;
		}
	},
	...
	reset : function() {
		this.input.value = this.input.getAttribute('data-value');
	},
	focusHandle : function() {
		var v = this.input.value;
		if (v === this.input.getAttribute('data-value')) {
			this.input.value = "";
		}
	},
	blurHandle : function() {
		var v = this.input.value.replace(/^\s*(.*?)\s+$/,"$1");
		if (!v.length) {
			this.reset();
		}
	}
...
InputSuggest.prototype = {
	...
	setPos: function(){
		...
		container.style.cssText =
			...
			+ pos[0] + 'px;top:'
			+ (pos[1]+(input.scrollTop + 27)) + 'px;width:'

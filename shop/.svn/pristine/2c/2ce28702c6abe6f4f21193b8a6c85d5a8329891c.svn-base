...
class common extends Model {
	...
	function get_order_stauts_config(){
		return array('10'=>'未付款','20'=>'已付款未处理','30'=>'已确认','40'=>'成功订单','50'=>'无效订单','60'=>'已删除','70'=>'已取消');
	}
	...
	function _get_order_detail($uid,$order_id){
		$sql ="select t_orders.to_id,t_categorys.name as cate_name,t_orders.to_uid,t_order_good.perprice as now_price,t_order_good.good_id,t_order_good.service_date,t_order_good.count,t_goods.* from t_categorys,t_orders,t_order_good,t_goods";
	...
	function change_order_status($order_sn,$wechat=''){
		...
		if($wechat){
		    ...
		    if($temp_order[0]->to_status == 10){
				$result = $this->tickets->update('orders',array('to_status'=>20),array('to_order_sn'=>$order_sn));
		    }
		}
		if($result){
...
AND t_shipping_address.tsa_district = t_area.area_id and t_shipping_address.tsa_uid=".$uid;
    ...
    function cookie_cart($good_id,$count,$building_id=0){
			...
			if ($temp_count ==0){
				...
				setcookie('goods_cart',serialize($temp_cart),time()+3600*24,'/');
			}
	...
	function _get_valid_weeks(){
		...
		if(($current_week == 6)||($current_week == 7)||($current_week == 5)){
			$week = array(
				0=>date('Y-m-d',strtotime(' +'.(8-$current_week).' days')),
				1=>date('Y-m-d',strtotime(' +'.(9-$current_week).' days')),
				2=>date('Y-m-d',strtotime(' +'.(10-$current_week).' days')),
				3=>date('Y-m-d',strtotime(' +'.(11-$current_week).' days')),
				4=>date('Y-m-d',strtotime(' +'.(12-$current_week).' days'))
			);
			...
			/*if ($today_flag){
				$week = array(
						...
						0=>date('Y-m-d',strtotime(' +5 days')),
						1=>date('Y-m-d',strtotime(' +6 days'))
		...
		function _get_all_valid_buildings($default_building_id=0,$uid = '',$main_index=0){
		    ...
		    foreach($service_buildings as $k=>$v){
	    		$sql = "select t_goods.*,t_good_supplier_buildings.* from t_good_supplier_buildings,t_goods where
				t_good_supplier_buildings.end_time > '".$now_time."' and t_good_supplier_buildings.good_id= t_goods.id and t_good_supplier_buildings.building_id = ".$v->id;

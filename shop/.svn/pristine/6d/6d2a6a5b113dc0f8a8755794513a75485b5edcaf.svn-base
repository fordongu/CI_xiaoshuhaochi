 //辅助函数
        ...
        {
            return "no4j44u3o49rj39r9n56fle44t323233";
        }
        ...
        {
            var banktype = "WX";
            var body = $('#order_title').html();//商品名称信息，这里由测试网页填入。
            var fee_type = "1";//费用类型，这里1为默认的人民币
            var input_charset = "UTF-8";//字符集，这里将统一使用GBK
            var notify_url = "http://www.7dianban.cn/index.php?c=service&m=weipay_success";//支付成功后将通知该地址
           // var out_trade_no = ""+getANumber();//订单号，商户需要保证该字段对于本商户的唯一性
            var out_trade_no = $('#hidden_order_sns').val();
            
            var partner = getPartnerId();//测试商户号
           
            var spbill_create_ip = "127.0.0.1";//用户浏览器的ip，这个需要在前端获取。这里使用127.0.0.1测试值
            var total_fee = parseFloat($('#alipay_amount').html())*100;//总金额。
            //var total_fee = 1;

...
<div class="tb-toolbar">
		<a href="/main/single_material_text" class="btn btn-small btn-primary">新增</a>		 
	</div>
	<table class="table table-hover table-striped">
		...
		<?php foreach ($materials as $key=>$val){?>
			<tr>
				...
				<td><a href="/main/single_material_text/<?php echo $val->tm_id;?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="javascript:void(0)" class="delete" data-id="<?php echo $val->tm_id;?>">删除</a></td>
...
<div class="pagination">
  <ul>
     ...
     <?php if($current_page=='1'){?><li class="disabled" ><span>上一页</span></li>
     <?php }else{?>
     <li><a href="/main/single_text/<?php echo $current_page-1;?>">上一页</a></li>
     <?php }?>
    <?php for ($i=1;$i<=$total;$i++){?>
       <?php if($current_page==$i){?><li class="active"><span><?php echo $i;?></span></li><?php }else{?>
       <li><a href="/main/single_text/<?php echo $i;?>"><?php echo $i;?></a></li><?php }?>
    <?php }?>
    <?php if ($current_page==$total){?>
    <li class="disabled"><span>下一页</span></li>
    <?php }else{?>
     <li><a href="/main/single_text/<?php echo $current_page+1;?>">下一页</a></li>
     <?php }?>
  </ul>
</div>
</body></html>